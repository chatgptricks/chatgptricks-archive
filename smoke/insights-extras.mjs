...
const ok = {};
const source = fs.readFileSync('public/insights.html','utf8');
...
ok['jsPDF loaded lazily'] = !/<script[^>]+jspdf/i.test(source);
ok['Clear chip in source'] = /mk\('Clear'/.test(source);
ok['Clear empties the set'] = /SEL=new Set\(\);buildChips\(\);render\(\)/.test(source);
ok['Follower strategy fetch is wired'] = source.includes('/api/insights/follower-growth');
ok['Follower attribution keeps the causal caveat'] = source.includes('never a confirmed Instagram follower conversion');
