...
import { trySsoSignIn, publishSsoCookie } from './sso';
import './promos.css';
...
function PromosApp() {
  const [user, setUser] = useState(undefined); const [authError, setAuthError] = useState(''); const [data, setData] = useState(EMPTY); const [selected, setSelected] = useState(null); const [filters, setFilters] = useState({ search: '', classification: '', review: '' }); const [loading, setLoading] = useState(false); const [error, setError] = useState('');
  useEffect(() => { trySsoSignIn().catch(() => {}); return onAuthStateChanged(firebaseAuth, value => { setUser(value || null); if (value) publishSsoCookie().catch(() => {}); }); }, []);
  ...
  return <main className="promo-shell"><header className="promo-header"><div><span className="promo-kicker">Competitive intelligence · private</span><h1>Promos</h1><p>Early signals of paid partnerships, affiliate offers and promotional CTAs.</p></div><button className="promo-signout" onClick={() => signOut(firebaseAuth)}>Sign out</button></header><section className="promo-toolbar"><input placeholder="Search client, product, page…" value={filters.search} onChange={e => setFilters({ ...filters, search: e.target.value })} /><select value={filters.classification} onChange={e => setFilters({ ...filters, classification: e.target.value })}><option value="">All signals</option><option value="disclosed">Disclosed</option><option value="likely">Likely</option><option value="needs_review">Needs review</option></select><select value={filters.review} onChange={e => setFilters({ ...filters, review: e.target.value })}><option value="">All reviews</option><option value="new">New</option><option value="reviewed">Reviewed</option><option value="dismissed">Dismissed</option></select><span className="promo-count">{items.length} opportunities</span></section>{error && <div className="promo-alert">{error}<button onClick={load}>Retry</button></div>}{loading && !data.items.length ? <div className="promo-empty">Loading opportunity signals…</div> : items.length ? <section className="promo-grid">{items.map(item => <Card key={`${item.account}:${item.shortcode}`} item={item} onSelect={setSelected} />)}</section> : <div className="promo-empty"><strong>No promotion signals yet.</strong><span>New competitor posts will appear here after ingestion and analysis.</span></div>}{selected && <Detail item={selected} onClose={() => setSelected(null)} onUpdate={next => { setSelected(next); setData(current => ({ ...current, items: current.items.map(item => item.account === next.account && item.shortcode === next.shortcode ? next : item) })); }} />}</main>;
}

ReactDOM.createRoot(document.getElementById('root')).render(<PromosApp />);
